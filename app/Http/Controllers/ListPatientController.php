    ...
    public function index (Request $request) 
    {
        ...
        if($request->session()->get('id') != NULL && $request->session()->get('pass') != NULL){
            ...
            WHERE
                ...
                $cnt = 0;
                $date ="";
                Log::debug($viewer_mst_data[0]['id']);
                ...
                foreach($array as $val){
                    ...
                    foreach($final_output_data as $value){
                        if($val['patient_id'] == $value['tbl3_patient_id']){
                            ...
                            $list_patient[$cnt]['RST'] = sprintf('%.1F', floatval($value['tbl3_mean_rsi']));
                            $list_patient[$cnt]['心拍数'] = sprintf('%.2F', floatval($value['tbl3_mean_hr']));
                            $list_patient[$cnt]['呼吸数'] = sprintf('%.1F', floatval($value['tbl3_mean_respr']));
                            $list_patient[$cnt]['CSRグレード'] = sprintf('%.1F', floatval($value['tbl3_mean_cvr']));
